...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset', type=str, default='electricity', help='Dataset to use for the experiments.')
    ...
    for T in range(num_steps_start, num_steps_end+1):
        commands.append(
            ...
            f"--runtime.output_dir=./outputs/spikernn_{args.dataset}_encoder=delta_T={T}"
        )
        commands.append(
            f"python -m SeqSNN.entry.tsforecast exp/forecast/spikernn/spikernn_{args.dataset}.yml "
            f"--network.encoder_type=repeat --network.num_steps={T} "
            f"--runtime.output_dir=./outputs/spikernn_{args.dataset}_encoder=repeat_T={T}"
        )
        commands.append(
            f"python -m SeqSNN.entry.tsforecast exp/forecast/spikernn/spikernn_{args.dataset}.yml "
            f"--network.encoder_type=conv --network.num_steps={T} "
            f"--runtime.output_dir=./outputs/spikernn_{args.dataset}_encoder=conv_T={T}"
        )
