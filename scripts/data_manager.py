...
if __name__ == '__main__':
    raw_str = 'data.dataset_name,data.file,data.horizon,data.last_label,data.normalize,data.raw_label,data.test_ratio,data.train_ratio,data.window,network.activation,network.cell_type,network.channel,network.channel_concat,network.class_strategy,network.common_thr,network.d_ff,network.d_model,network.depths,network.dilation,network.dim,network.dropout,network.e_layers,network.emb_dim,network.emb_type,network.embed,network.encoder_type,network.factor,network.freq,network.gpu_id,network.grad_slope,network.heads,network.hidden_size,network.injection_type,network.input_size,network.is_bidir,network.k_c,network.k_t,network.kernel_size,network.layers,network.max_length,network.n_cluster,network.n_heads,network.neuron_pe_scale,network.num_layers,network.num_levels,network.num_pe_neuron,network.num_steps,network.output_attention,network.pe_mode,network.pe_type,network.position_embedding,network.qk_scale,network.qkv_bias,network.stride,network.use_all_random,network.use_all_zero,network.use_cluster,network.use_ste,network.weight_file,runner.aggregate,runner.batch_size,runner.beta,runner.checkpoint_dir,runner.d_model,runner.denormalize,runner.early_stop,runner.loss_fn,runner.lower_is_better,runner.lr,runner.max_epoches,runner.metrics,runner.model_path,runner.n_cluster,runner.n_vars,runner.network,runner.observe,runner.optimizer,runner.out_ranges,runner.out_size,runner.output_dir,runner.seq_len,runner.task,runner.use_cluster,runner.valid_variates,runner.weight_decay,runtime.checkpoint_dir,runtime.debug,runtime.output_dir,runtime.seed,runtime.tb_log_dir,runtime.use_cuda'
